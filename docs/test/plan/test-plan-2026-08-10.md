...
2. 验证错误处理规范落地：响应体统一 `{code, message, data}`，`code` = HTTP 状态码；401/403/409/500 映射正确，不透传内部细节（见 docs/conventions/backend-conventions.md）。
...
- [ ] 数据库已按 `docs/database/db-schema.sql` 建表
...
服务已在运行，直接打真实端点（路径以实际 controller 为准，见 docs/spec/openapi.yaml 核对）：
