...
**分层单体架构（Layered Monolith）** — 单一 Spring Boot 进程运行后端服务，前端为独立 Vue 3 SPA（开发中，详见 [v5. 前端模块设计](#5-前端模块设计)），AI Agent 为独立 FastAPI 进程。三部分通过 REST API 通信。
...
| JWT 管理 | 自签发 + Redis 白名单 | 支持服务端登出，无需 Refresh Token |
| 响应结构 | `Result<T>` 统一包装 | 接口统一，便于前端消费 |
...
| **前端框架** | Vue 3 + TypeScript | 3.4+ | SPA UI（开发中） |
| **构建工具** | Vite | 5.3+ | 前端构建 |
...
| **工具库** | Commons Codec | 1.16+ | SHA256 摘要（Token 哈希） |
...
```
AnimeTracker/
├── frontend/                          # ★ Vue 3 前端 SPA（重构中）
│   └── DESIGN.md                      #   Apple 风格设计规范（Claude Design）
│
├── backend/                           # ★ 后端代码
│   ├── api/                           #   Java Spring Boot 单体应用
│   │   ├── pom.xml
│   │   ├── src/main/java/top/zhaizz/animetracker/
│   │   │   ├── AppApplication.java           # 启动入口（含 MapperScan）
│   │   │   ├── common/                       # 公共模块
│   │   │   │   ├── result/                   #   统一响应体
│   │   │   │   │   ├── Result.java           #     通用响应 {code, message, data}
│   │   │   │   │   └── PageResult.java       #     分页响应 {content, total, page, size}
│   │   │   │   ├── exception/                #   异常体系
│   │   │   │   │   ├── BizException.java     #     业务异常（带 code/message）
...
│   │   │   │   └── OpenApiConfig.java        # OpenAPI + Knife4j
│   │   │   ├── security/                     # JWT 安全
...
│   │   ├── storage/                  #     会话存储（骨架，待实现）
│   │   │   ├── session_store.py      #     抽象基类 + 工厂
│   │   │   ├── redis_store.py        #     Redis 实现（待实现）
│   │   │   └── sqlite_store.py       #     SQLite 实现（待实现）
│   │   └── tests/
│   │       └── __init__.py
│   │
│   ├── data/                         #   数据层
...
│   ├── superpowers/plans/
│   │   └── 2026-07-05-auth-token-redis.md
│   ├── superpowers/out/
│   │   ├── tech-spec.md              #   本文件
...
| 类 | 职责 | 关键 API |
|----|------|----------|
| `Result<T>` | 统一响应体（替代旧 `ApiResponse`） | `success(data)`, `success()`, `error(code, msg)`, `fail(msg)` |
...
| `GlobalExceptionHandler` | 全局异常处理 | `@RestControllerAdvice`，捕获 BizException / MethodArgumentNotValidException / ConstraintViolationException / NoHandlerFoundException / 通用 Exception |
| `RedisClient` | Redis 操作封装 | `set(key, value, ttl?, unit?)`, `get(key)`, `del(key)`, `exists(key)` |
...
**`RedisClient` 实现说明：**
- 基于 `StringRedisTemplate`（非 `RedisTemplate<String, Object>`），所有值存为字符串。
- 用于认证令牌白名单存储，由 `AuthService` 和 `JwtAuthenticationFilter` 调用。
- 不提供 `@Cacheable` 注解支持（无 `CacheConfig` / `@EnableCaching`）。

### 4.2 Config 模块

| 配置类 | 职责 | 关键配置 |
|--------|------|----------|
| `SecurityConfig` | Spring Security | 无状态 Session，BCrypt 密码编码，`JwtAuthenticationFilter` 前置 |
| `JwtTokenProvider` | JWT 令牌 | HMAC-SHA256 签名，从 `jwt.secret` 配置读取密钥，`jwt.expiration` 控制过期时间（默认 24h） |
| `JwtAuthenticationFilter` | JWT 过滤器 | 从 `Authorization: Bearer <token>` 提取 → 解析 → Redis 白名单校验 → 注入 SecurityContext |
| `CorsConfig` | CORS | 允许 `/api/**` 所有 Origin (`allowedOriginPatterns: "*"`)，支持凭证 |
| `MyBatisPlusConfig` | MyBatis-Plus | 分页拦截器（MySQL 方言），单页最大 100 条，越界返回空页 |
| `OpenApiConfig` | API 文档 | OpenAPI 3.0 配置，标题 "Anime Tracker API"，BearerAuth 安全方案 |

**配置类变更记录：**
- `Knife4jConfig` → 整合为 `OpenApiConfig`（SpringDoc 原生配置 + Knife4j 增强），Knife4j 配置保留在 `application.yml` 中。
- `CacheConfig`（旧）→ 移除，当前未使用 `@EnableCaching` / `RedisTemplate` 注解式缓存。

### 4.3 User 模块

| 组件 | 职责 | API 路径 |
|------|------|----------|
| `AuthController` | 注册/登录/登出 | `POST /api/user/auth/register`, `POST /api/user/auth/login`, `GET /api/user/auth/logout` |
| `UserController` | 个人信息 | `GET /api/user/me`, `PUT /api/user/me` |
| `AdminUserController` | 用户管理 | `GET /api/admin/users`, `PUT /api/admin/users/{id}/role` |

**认证流程：**
```
Register: 校验参数 → 唯一性检查(username) → BCrypt 加密 → 写入 DB → 生成 JWT → Redis 白名单 → 返回 {token, user}
Login:    查找用户 → BCrypt 验证 → 生成 JWT → Redis 白名单(ttl=24h) → 返回 {token, user}
Logout:   提取 Token → SHA256 摘要 → 删除 Redis 白名单条目 → 返回
Auth:     Authorization: Bearer <token> → JWT 签名验证 → SHA256 摘要 → 查询 Redis 白名单 → 注入 SecurityContext
```
...
**DTO 和 VO 说明：**
- 请求 DTO 统一位于 `common.dto` 包（`LoginDTO`, `RegisterDTO`, `UpdateUserDTO`, `UpdateRoleDTO`）
- 返回 VO 统一位于 `common.vo` 包（`LoginVO`, `UserVO`）
- `UserConverter` 处理 `User` ↔ `UserVO` 转换及 `UpdateUserDTO` → 实体合并
...
> **当前状态：** 前端 Vue 3 SPA 源码已移出主仓库，正在进行 Apple 风格设计重构。以下为架构规划，具体实现待定。
...
> **注意：** `RedisSessionStore` 和 `SqliteSessionStore` 当前为骨架实现（`# Phase 3 实现`），功能待开发。`SessionStore` 抽象接口和工厂方法 `create_session_store()` 已完成。
...
| 缓存键模式 | 存储数据 | TTL | 失效时机 |
|-----------|---------|-----|---------|
| `auth:token:{sha256(token)}` | 用户 ID (String) | 24h（与 JWT 过期一致） | 用户登出时 `del` |
| `subject:detail:{id}` | SubjectDetailVO (JSON) | 30 分钟 | 条目被更新/删除时（待实现 `@CacheEvict`） |
| `subject:search:{keyword}:{page}:{size}` | 搜索结果 (JSON) | 10 分钟 | 数据导入后批量失效（待实现） |
| `ai:session:{session_id}` | 会话数据 (Hash) | 7 天 | 显式删除会话时（待实现） |

### 8.2 缓存分层

```
Redis 实例
├── db=0    → AI Agent 会话缓存 (ai:session:*)       [待实现]
├── db=1    → Java API 数据缓存 + 认证令牌白名单
│   ├── auth:token:*    (认证白名单，已实现)
│   ├── subject:detail:* (数据缓存，待实现)
│   └── subject:search:* (数据缓存，待实现)
└── db=2    → 预留
```

> **当前实现状态：** `auth:token:*` 认证白名单已实现上线。`subject:detail:*` / `subject:search:*` 数据缓存和 `ai:session:*` 会话缓存为设计规划，待后续开发。

### 8.3 认证令牌白名单流程

```
登录: 生成 JWT → SHA256(token) → Redis SET auth:token:{hash} userId TTL 24h
鉴权: 提取 JWT → 校验签名 → SHA256(token) → Redis EXISTS auth:token:{hash} → 通过/拒绝
登出: 提取 JWT → SHA256(token) → Redis DEL auth:token:{hash}
```

- 使用 SHA256 摘要存储 Token 哈希值而非原始 Token，避免敏感数据明⽂存储
- 每次请求都经过 Redis 白名单校验，服务端登出即时生效
- JWT 签名过期后，Redis 中对应的 key 也会因 TTL 到期自动清理

### 8.4 穿透防护

- 查询未命中时，缓存空值（有效期 1 分钟）（待实现）
- 防止恶意查询不存在的主键导致 MySQL 压力

### 8.5 降级策略

- Redis 连接失败 → 捕获 `RedisConnectionFailureException`
- 认证白名单降级：Redis 不可用时拒绝所有需要认证的请求（安全优先）
- API 数据缓存降级为直接查询 MySQL（待实现）
- AI 会话降级为 SQLite 本地存储（待实现）
- 应用启动不因 Redis 不可用而阻断
...
- 密钥通过 `application.yml` 配置 `jwt.secret` 提供（生产环境建议通过环境变量注入）
- 过期时间：24 小时（86400000 ms），通过 `jwt.expiration` 配置
- Token 通过 `Authorization: Bearer <token>` 请求头传递
- 服务端通过 Redis 白名单控制 Token 有效性，支持即时登出

### 9.2 Redis 白名单机制

```
┌─────────────┐     ┌──────────────────┐     ┌─────────────┐
│  登录/注册   │────→│ JwtTokenProvider │────→│ Redis 白名单 │
│             │     │ generateToken()  │     │ SET + TTL   │
└─────────────┘     └──────────────────┘     └─────────────┘
                                                      │
┌─────────────┐     ┌──────────────────┐              │
│  每次请求    │────→│ JwtAuthFilter   │────→ Redis EXISTS
│ Auth Header │     │ resolve+validate │     (白名单校验)
└─────────────┘     └──────────────────┘              │
                                                      │
┌─────────────┐     ┌──────────────────┐              │
│  登出        │────→│ AuthService     │────→ Redis DEL
│             │     │ logout()         │     (移除白名单)
└─────────────┘     └──────────────────┘
```
...
| 路径模式 | 方法 | 权限 |
|---------|------|------|
| `POST /api/user/auth/register` | POST | `permitAll()` |
| `POST /api/user/auth/login` | POST | `permitAll()` |
...
| `/doc.html`, `/v3/api-docs/**`, `/swagger-ui/**` | GET | `permitAll()` |

> **注意：** `/api/user/**` 为兜底规则（`authenticated()`），已通过前置精确路径放行了公开接口。认证失败时返回 401。

### 9.4 密码安全

- 使用 BCrypt 密码编码器（Spring Security 默认）
- 数据库不存储明文密码
- 注册时进行密码强度校验（长度 ≥ 6 位，通过 `@Size(min=6)` 注解）

### 9.5 其他安全措施

- 无状态 Session（不创建 HttpSession）
- CORS 白名单（开发环境允许所有 Origin，生产环境按需限制）
- 用户名唯一性校验（防止重复注册）
- `JWT_SECRET`（通过 `jwt.secret` 配置）至少 256 位，使用 `Keys.hmacShaKeyFor()` 确保密钥强度
...
| 任务 | 内容 | 估算 |
|------|------|------|
| T2.1 Common 模块 | Result / PageResult / BizException / ErrorType / GlobalExceptionHandler / RedisClient | 0.5 天 |
| T2.2 Config 模块 | SecurityConfig / CorsConfig / MyBatisPlusConfig / OpenApiConfig | 0.5 天 |
| T2.3 Security 层 | JwtTokenProvider / JwtAuthenticationFilter（含 Redis 白名单）/ UserPrincipal | 0.3 天 |
| T2.4 User 模块 | User 实体 + Mapper + DTO/VO + Converter + Service + Controller(3 个，含 logout) | 0.7 天 |
| T2.5 Subject 模块 | Subject/Episode/SubjectTag/ImportRecord 实体 + Mapper + DTO/VO + Converter + Service + Controller(4 个) | 1.0 天 |
...
**目标：** AI Agent 会话 Redis 实现 + API 数据缓存 + 认证令牌白名单
...
| 任务 | 内容 | 估算 | 状态 |
|------|------|------|------|
| T6.1 | AI 会话存储重构为抽象接口 + Redis 实现 | 0.3 天 | ✅ 接口就绪，实现为骨架 |
| T6.2 | Java API Subject 缓存（@Cacheable + @CacheEvict） | 0.4 天 | ⏳ 待实现 |
| T6.3 | 缓存穿透防护 + 缓存清除 API | 0.2 天 | ⏳ 待实现 |
| T6.4 | 降级测试（Redis 不可用时系统表现） | 0.1 天 | ⏳ 待实现 |
| T6.5 | **JWT 令牌 Redis 白名单 + 登出功能** | 0.3 天 | ✅ 已实现 |
...
| GET | `/api/user/auth/logout` | 用户登出（清除 Redis 白名单） | 是 |
...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/chat/health` | 健康检查 | 无 |
| POST | `/api/chat` | 发送聊天消息（SSE 流式） | 无 |
| GET | `/api/chat/{session_id}/messages` | 获取会话历史 | 无 |
| POST | `/api/chat/sessions` | 创建会话 | 无 |
| GET | `/api/chat/sessions` | 会话列表 | 无 |
| DELETE | `/api/chat/sessions/{session_id}` | 删除会话 | 无 |
| GET | `/api/chat/config` | Agent 配置信息 | 无 |
...
| `jwt.secret` | `your-256-bit-secret-...` | JWT 签名密钥 | Java API |
| `jwt.expiration` | `86400000` | JWT 过期毫秒数 | Java API |
...
> **配置说明：** `application.yml` 中 Java API 的数据库和 Redis 配置使用了自定义命名空间 `zzz.*`（如 `zzz.datasource.host`、`zzz.data.redis.host`），该命名空间变量与上表中的环境变量一一对应。JWT 配置通过 `jwt.secret` 和 `jwt.expiration` 读取，支持在 `application-local.yml` 中覆盖。
...
|------|------|----------|
| 1.0 | 2026-07-03 | 初始版本 |
| 2.0 | 2026-07-03 | 结构调整完成 |
| **3.0** | **2026-07-06** | **同步代码变更：** 统一响应体从 `ApiResponse` 重构为 `Result<T>`；新增 Redis 认证令牌白名单机制；新增登出接口；新增 `RedisClient` 工具类；Config 模块移除 `CacheConfig`，`Knife4jConfig` → `OpenApiConfig`；DTO/VO/Entity 统一至 `common` 包；前端 SPA 源码移出主仓库进行设计重构 |
