...
3. **失败分类**：
   ...
   - 代码问题 → 记缺陷（复现步骤、实际/期望响应、严重度）于报告 §三，**只记录不修复**。
4. **测试账号**：client 用 `test1/123456`，admin 用 `admin/123456`；登录失败先查库确认账号存在且邮箱已验证，**不要注册新账号**（避免撞限流）。
5. **安全**：报告/输出中不得出现 token、密钥；测试账号密码仅在登录命令中使用，不写入报告；不修改业务代码。
6. **产出**：执行完按 §7 模板写报告，保存为 `docs/test-report-<YYYYMMDD>.md`，然后 `git add` + `git commit`（commit 遵循 `.gitmessage` 规范：`类型(scope): 中文描述`）+ `git push`。
