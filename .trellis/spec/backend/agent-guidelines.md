...
- [Agent 编排与流式协议](#agent-编排与流式协议)
- [Agent 角色、提示词与流式输出契约](#agent-角色提示词与流式输出契约)
...
- `ADMIN` 直接进入 `admin_agent`，其当前工具集不含 RAG；角色工具、Prompt 缓存、语言与日期的完整现状见 [Agent 运行与提示词契约](./agent-guidelines.md#agent-角色提示词与流式输出契约)。
