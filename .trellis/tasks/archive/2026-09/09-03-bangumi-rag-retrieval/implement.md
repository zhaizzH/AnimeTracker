...
- Phase 8：2026-09-07 真实回放确认 v1 120/120 通过；五份 v1 gate 报告（含 20 条人工证据检查）均已通过，v1 ACTIVE release 已激活。2026-09-09 用户确认 24 小时灰度观察与 release/功能开关回滚通过。真实库在 migration-003 后为 23 张表，Redis 8.8.0 实际使用 DB 1。
...
- [x] 在同一 release candidate 上运行数据质量、双投影容量、120-case 正式 eval、延迟与至少 20 条人工证据检查，所有报告绑定同一 index/profile version；eval 明确为 `RELEASE_CANDIDATE`，五份报告已通过 gate 并完成激活。
- [x] 覆盖 MySQL FULLTEXT、Redis Vector Set、版本错配、Embedding、Business、MinIO 故障矩阵，证明 fail-closed 或既定降级行为。
- [x] 小流量激活新 MySQL release 与 RAG，完成 24 小时观察；用户确认异常时 release 与功能开关回切通过。
- [x] 指标稳定后更新任务 README、运行审计与灰度确认记录；旧索引/旧表删除仍需另行确认和规划。
...
- 120-case 端到端评测、五份 gate 报告和 24 小时灰度/回滚确认均已完成；`research/gray-v1-confirmation.md` 记录本次用户确认，历史阻断条件保留在审计报告的历史段落中。
...
- 正式发布链当前状态为：五份报告已 gate PASS → v1 已人工确认并激活 → 24 小时灰度与回滚确认通过。后续版本仍必须重复同一 gate、激活与灰度流程。
