...
2. AC4：结构化 `person_ids`、`character_ids`、`actor_ids`、`relation_subject_ids` 与受控 `entity_name/entity_kind` 已接入 Python `RetrievalQuery`、RAG 工具和 Business `/resolve`；名称从版本化实体 shadow index 解析，PERSON/CHARACTER 同名候选按名称约束取并集，ACTOR 保留关系语义；`RELATION_SUBJECT` 沿 `subject_relation` 双向扩展，Redis Top-50 不足时改走精确权威回查，Redis 与 Business fallback 均执行 allowlist，解析异常 fail-closed。完整自然语言查询规划、关系名称解析和“找某声优参与作品”的自由表达仍需后续切片。
...
- `backend/agent` 全量：**216 passed，1 deselected**；被排除的是依赖 Windows `tmp_path` 的自定义 golden loader 测试，环境临时目录 ACL 返回 WinError 5；同一逻辑使用工作区文件手工验证通过。
- `backend/agent` `compileall -q app jobs`：通过。
- `backend/business` `mvn -B test`：**30 passed，BUILD SUCCESS**。
- `git diff --check`：通过（仅 CRLF 转换提示）。
- Phase 7 结构化实体筛选与名称解析：定向测试 **59 passed**，RAG/适配器范围 **73 passed**；Business HTTP `/resolve` 契约、`RELATION_SUBJECT` 服务、名称转义/类型映射和 fail-closed 测试已覆盖。
