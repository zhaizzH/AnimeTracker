...
from app.adapters.llm.embeddings import EmbeddingUnavailable
from app.adapters.redis.subject_index import vector_bytes
...
def _business_search(_query, **_kwargs):
    return []
...
def test_embedding_failure_falls_back_to_authoritative_lexical_results():
    ...
    service = RagRetrievalService(
        ...
        business_search=_business_search,
    )

    result = service.retrieve(RetrievalQuery(semantic_query="治愈", keywords=["治愈"]))
...
def test_semantic_embedding_failure_uses_escaped_text_not_a_wildcard_search():
    ...
    service = RagRetrievalService(index, FailingEmbedding(), authority_lookup=_authority, business_search=_business_search)

    result = service.retrieve(RetrievalQuery(semantic_query="*)|(@nsfw:{true}"))
...
def test_structured_query_without_semantics_uses_lexical_filter_search():
    ...
    result = service.retrieve(RetrievalQuery(year_from=2024))
...
def test_meta_tag_filter_uses_a_single_escaped_tag_clause():
    ...
    index = CapturingIndex(lexical=[{"subject_id": 7, "title": "动画 7"}], semantic=[])
    service = RagRetrievalService(index, VectorEmbedding(), authority_lookup=_authority, business_search=_business_search)

    service.retrieve(RetrievalQuery(meta_tags=["科幻,太空", "校园|日常"]))
...
def test_business_failure_never_returns_unvalidated_redis_candidates():
    ...
    result = service.retrieve(RetrievalQuery(keywords=["治愈"]))
...
def test_relaxation_continues_when_first_redis_candidates_fail_authority_gate():
    ...
    service = RagRetrievalService(RelaxingIndex(), VectorEmbedding(), authority_lookup=authority, business_search=_business_search)

    result = service.retrieve(RetrievalQuery(keywords=["治愈"], score_min=8.0))
...
def test_user_profile_vector_reranks_candidates_by_cosine_similarity():
    ...
    result = service.retrieve(RetrievalQuery(keywords=["治愈"]), preference=preference)
...
def test_missing_profile_marks_cold_start_notice():
    ...
    result = service.retrieve(RetrievalQuery(keywords=["治愈"]), personalization_missing=True)
