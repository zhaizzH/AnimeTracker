...
export interface SubjectListItem { id: number; name: string; nameCn?: string | null; image?: string | null; score: number; rank: number; eps: number; airDate?: string | null; type: number; airWeekday: number; collectionTotal: number }
...
export interface EpisodeVO { id: number; subjectId: number; type: number; sort: number; name: string; nameCn?: string | null; duration?: string | null; airdate?: string | null; description?: string | null; status: 'Air' | 'Today' | 'NA' }

export interface CollectionVO { id: number; subjectId: number; type: CollectionType; rate: number; epStatus: number; subject: SubjectListItem }
...
export interface HotItemVO { id: number; name: string; nameCn?: string | null; image?: string | null; collectionCount: number }

export interface WishlistResult { state: 'ADDED' | 'ALREADY_COLLECTED'; existingType?: CollectionType }
export interface PreviewItemVO { subjectId: number; subjectName: string; currentEpStatus: number; targetEpStatus: number; completedAfterUpdate: boolean; suggestMarkAsWatched: boolean }
...
export interface ExecutedItemVO { subjectId: number; subjectName: string; currentEpStatus: number; targetEpStatus: number; completedAfterUpdate: boolean; suggestMarkAsWatched: boolean }
export interface SkippedItemVO { subjectId: number; subjectName: string; currentEpStatus: number; targetEpStatus: number; reason: string }
