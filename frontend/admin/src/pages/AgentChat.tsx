...
export default function AgentChat() {
  ...
  const { isLoading: sessionsLoading, data: sessions, isError: sessionsError } = useQuery<Session[]>({
    ...
    queryFn: () => agentApi.adminSessions() as Promise<Session[]>,
    retry: 1,
  });
  ...
  const loadHistory = async (sessionId: string) => {
    ...
    try {
      const history = await agentApi.adminHistory(sessionId) as any;
      const msgs = history || [];
  ...
  const createSession = async () => {
    try {
      const result = await agentApi.adminCreateSession() as any;
  ...
  const deleteSession = async (sessionId: string) => {
    try {
      await agentApi.adminRemoveSession(sessionId);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await agentApi.adminStream(sessionId!, userMessage.content, accessToken);

      // 流式请求不走 axios 拦截器，401 时直接跳登录（admin 无 token 刷新流程）
