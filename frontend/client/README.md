...
- 支持“推荐后加入想看”和“更新本周追番进度”等跨轮次操作
- 写操作先展示预览，只有用户明确确认后才执行；取消或预览过期不会修改收藏数据

典型交互链路：**描述偏好 → Agent 调用实时番剧与收藏工具 → 流式返回推荐 → 用户选择条目 → 预览变更 → 确认执行**。待确认状态由 Agent 服务持久化，前端只负责呈现对话和继续发送用户决定。
