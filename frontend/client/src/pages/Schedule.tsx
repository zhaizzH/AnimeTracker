...
export default function Schedule() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['schedule', year, quarter, active], queryFn: () => subjectsApi.schedule({ year, quarter, weekday: Number(active), size: 100 }) });
  const items = WEEK.map((label, i) => ({ key: String(i), label, children: <SubjectGrid items={(data?.content ?? []).filter((s) => s.airWeekday === i)} loading={isLoading} onItemClick={(s) => navigate(`/subject/${s.id}`)} /> }));
  return (
